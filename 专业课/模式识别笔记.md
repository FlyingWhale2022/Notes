...
$\Sigma=\frac{1}{N-1}\sum_{i=1}^{N}(x-\mu)(x-\mu)^{\mathrm{T}}\quad\mu=\frac{1}{N}\sum_{i=1}^{N}x$

- 求解其特征方程，对特征值从大到小进行排队，并选定前 m 个特征值及对应的特征向量
- 依次将 m 个特征向量作为转换矩阵对样本的原特征进行转换从而生成 m 个新的主成分特征
