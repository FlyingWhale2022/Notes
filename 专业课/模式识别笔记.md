...
$\Sigma=\frac{1}{N-1}\sum_{i=1}^{N}(\vec{x_i}-\mu)(\vec{x_i}-\mu)^{T}$

其中，$\vec{x_i}$ 为单个样本，$\mu$ 为样本的均值向量。

还可以用更便捷的方法计算协方差矩阵，

所有样本减去均值向量（即： $\vec{x_i}-\mu$ ），然后组成矩阵 $X$，协方差矩阵：
...
$R=E[XX^{\mathrm{T}}]\approx\frac{1}{N-1}\sum_{j=1}^{N}X_{j}X_{j}^{\mathrm{T}}$ 
