...
设 $x^*$ 为 $\varphi\left(x\right)$ 的不动点，$\varphi^{\prime}(x)$ 在 $x^*$ 某领域上 $S$ 上连续，且在该领域上能找到一点 $\xi$ ，使 $\varphi^{\prime}(\xi)<1$，则迭代格式局部收敛。

> 通俗来讲，就是在 $x^*$ 附近找一个点近似代替
...
> 结论：基于不动点的收敛迭代基本都是线性一阶的
> 
> 注意：$x^*$ 是未知的，因此通常用 $x^*$ 附近的值来近似替代 $\varphi(x^{*})$，见下面例题
...
[常微分方程基本概念介绍](https://www.bilibili.com/video/BV12b411W7DN?t=7.8)
